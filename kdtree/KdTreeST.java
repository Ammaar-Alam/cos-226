...
public class KdTreeST<Value> {
    ...
    private static int compareTo(Point2D a, Point2D b, boolean orientation) {
        return orientation ? Double.compare(a.x(), b.x()) :
    ...
    public void put(Point2D p, Value val) {
        ...
        if (p.x() < 0 || p.x() > 1 || p.y() < 0 || p.y() > 1) {
            throw new IllegalArgumentException(
                    "poitn coordinates should fall "
                            + "within the unit square [0, 1] x [0, 1]");
        }
        root = put(root, p, val, 0, 0, 1, 1, VERTICAL);
    }

    // helper method to insert a new node into the tree
    private Node put(Node h, Point2D p, Value val,
        ...
        if (cmp < 0) {
            if (orientation) {
                h.lb = put(h.lb, p, val, xmin, ymin, h.p.x(), ymax, !orientation);
            }
            else {
                h.lb = put(h.lb, p, val, xmin, ymin, xmax, h.p.y(), !orientation);
            }
        }
        else if (cmp > 0 || (cmp == 0 && compareTo(p, h.p, !orientation) != 0)) {
            if (orientation) {
                h.rt = put(h.rt, p, val, h.p.x(), ymin, xmax, ymax, !orientation);
            }
            else {
                h.rt = put(h.rt, p, val, xmin, h.p.y(), xmax, ymax, !orientation);
            }
        }
        else {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            StdOut.println("put() failed to throw an "
